...
//            DDLog(@"labRect%ld_%@",i,NSStringFromCGRect(labRect));
            UILabel * label = [self createLabelWithRect:labRect text:titles[i] textColor:UIColor.blackColor tag:kTAG_LABEL+i type:@2 font:17 backgroudColor:UIColor.whiteColor alignment:NSTextAlignmentCenter hasGesture:YES target:self aSelector:@selector(tapView:)];
        ...
        if (self.indicatorColor) {
            ...
            lineViewHori.backgroundColor = UIColor.themeColor;
    ...
    for (UILabel * view in self.labelMarr) {
        ...
        if (selectedIndex != (lable.tag - kTAG_LABEL)) {
            ...
            if ([self.titleColorSelected isKindOfClass:[UIColor class]]) {
                ...
                lable.textColor = UIColor.themeColor;
...
- (UILabel *)createLabelWithRect:(CGRect)rect text:(NSString *)text textColor:(UIColor *)textColor tag:(NSInteger)tag type:(NSNumber *)type font:(CGFloat)fontSize backgroudColor:(UIColor *)backgroudColor alignment:(NSTextAlignment)alignment hasGesture:(BOOL)hasGesture target:(id)target aSelector:(SEL)aSelector;
{
