...
#import "BN_ExtendView.h"

static NSString * const kExtendKey_btn = @"kExtendKey_BTN";
static NSString * const kExtendKey_rect = @"kExtendKey_rect";
static NSString * const kExtendKey_rectOrigin = @"kExtendKey_rectOrigin";


@interface BN_ExtendView ()
...
- (void)handleActionBtn:(UIButton *)sender{
    ...
    UIView * fromView = _isOpen == YES ? self.btnFront : self.btnBack;
    UIView * toView = _isOpen == YES ? self.btnBack : self.btnFront;
    ...
    switch ([_direction integerValue]) {
        case 0://上
        {
            options = _isOpen == YES ? UIViewAnimationOptionTransitionFlipFromTop : UIViewAnimationOptionTransitionFlipFromBottom;
            
        }
            break;
        case 1://左
        {
            options = _isOpen == YES ? UIViewAnimationOptionTransitionFlipFromLeft : UIViewAnimationOptionTransitionFlipFromRight;
            
        }
            break;
        case 2://下
        {
            options = _isOpen == YES ? UIViewAnimationOptionTransitionFlipFromBottom : UIViewAnimationOptionTransitionFlipFromTop;
            
        }
            break;
        case 3://右
        {
            options = _isOpen == YES ? UIViewAnimationOptionTransitionFlipFromRight : UIViewAnimationOptionTransitionFlipFromLeft;
...
- (void)updateBtnFrameIsOpen:(BOOL)isOPen{
    ...
    for (NSDictionary * dict in self.btnList) {
        ...
        [UIView animateWithDuration:0.15 animations:^{
            btn.frame = isOPen == YES ? [dict[kExtendKey_rect] CGRectValue] : [dict[kExtendKey_rectOrigin] CGRectValue];
