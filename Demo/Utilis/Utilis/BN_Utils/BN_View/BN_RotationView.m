...
@implementation BN_RotationView
...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        _layerFront = [CAShapeLayer layerWithRect:frame path:path strokeEnd:0.5 fillColor:UIColor.clearColor strokeColor:UIColor.redColor lineWidth:1];
...
//        self.layerFront.anchorPoint = CGPointMake(0.5, 0.5);//锚点设置为图片中心，绕中心抖动
        ...
        _layerBack = [CAShapeLayer layerWithRect:frame path:path strokeEnd:1 fillColor:UIColor.clearColor strokeColor:colorBack lineWidth:2];
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context{
    if ([keyPath isEqualToString:@"image"]) {
        self.layerFront.strokeColor = self.isColorFollow == YES ? self.imgView.image.mostColor.CGColor : UIColor.greenColor.CGColor;
