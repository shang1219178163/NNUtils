...
@implementation BN_RequestHelper

/** 还有参数uuid token
 获取业务接口地址+参数
 */
+ (NSString *)getInterfaceUrlWithMessageName:(NSString *)messageName{
    NSString *kHTTP_URLMiddle = @"i=42&c=entry&do=fengmian&m=android&source=ios";//通用参数
    ...
    //token
    NSString * tokenRequest = [BN_RequestHelper getTokenRequestMessageName:[NSString stringWithFormat:@"r=%@",messageName]];
