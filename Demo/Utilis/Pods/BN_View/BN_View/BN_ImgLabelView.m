...
@implementation BN_ImgLabelView
...
-(UILabel *)labelTitle{
    ...
    if (!_labelTitle) {
        _labelTitle = [self createLabelWithRect:CGRectZero text:@"" textColor:nil tag:kTAG_LABEL type:@0 font:kFZ_Third backgroudColor:nil alignment:NSTextAlignmentLeft];;
    }
    return _labelTitle;
}
...
{
    BN_ImgLabelView *view = [[self alloc]init];
...
- (UILabel *)createLabelWithRect:(CGRect)rect text:(NSString *)text textColor:(UIColor *)textColor tag:(NSInteger)tag type:(NSNumber *)type font:(CGFloat)fontSize  backgroudColor:(UIColor *)backgroudColor alignment:(NSTextAlignment)alignment{
