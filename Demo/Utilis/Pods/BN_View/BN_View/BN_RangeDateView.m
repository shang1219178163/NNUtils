...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        _dateStart = NSDate.date.now;
        _dateEnd = NSDate.date.now;;
...
- (void)createControls{
    ...
    for (NSInteger i = 0; i < self.titleList.count; i++) {
        ...
        UILabel * label = [UIView createLabelWithRect:rect text:title textColor:nil tag:kTAG_LABEL+i type:@2 font:15 backgroudColor:UIColor.whiteColor alignment:NSTextAlignmentCenter];
        if (i%2 == 1) {
            label.text = [title toDateShort];
            label.layer.borderColor = UIColor.lightGrayColor.CGColor;
            label.layer.borderWidth = 0.5;
//            label.backgroundColor = UIColor.backgroudColor;
...
//        }
//
//        UILabel * label = [UIView createLabelWithRect:rect text:title textColor:nil tag:kTAG_LABEL+i type:@2 font:16 backgroudColor:UIColor.whiteColor alignment:NSTextAlignmentCenter];
//        if (i%2 == 1) {
//            label.text = [title toDateShort];
...
- (void)handlActionSender:(UITapGestureRecognizer *)tap{
    ...
    if (dateBegin.length != 10) dateBegin = NSDate.date.now;
...
- (void)createDatePick:(id)date tag:(NSInteger)tag{
    ...
    if ([date isKindOfClass:[NSString class]]) date = [self dateWithString:date];
