...
@interface BN_AddPictureVC ()<TZImagePickerControllerDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate>
...
@implementation BN_AddPictureVC

+(instancetype)sharedInstance{
    static BN_AddPictureVC *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[BN_AddPictureVC alloc]init];
...
+(instancetype)allocWithZone:(struct _NSZone *)zone{
    static BN_AddPictureVC *instance = nil;
            ...
            if (error) {
                ...
                [[TZImageManager manager] getCameraRollAlbum:NO allowPickingImage:YES needFetchAssets:YES completion:^(TZAlbumModel *model) {
                    [[TZImageManager manager] getAssetsFromFetchResult:model.result allowPickingVideo:NO allowPickingImage:YES completion:^(NSArray<TZAssetModel *> *models) {
                        ...
                        //处理
                        if (self.allowCropImage == YES) { // 允许裁剪,去裁剪
