...
@implementation WHKTableViewSixCell
...
-(void)layoutSubviews{
    ...
    self.imgViewRight.frame = CGRectMake(self.width - arrowSize.width - kX_GAP, (self.height - arrowSize.height)/2.0, arrowSize.width, arrowSize.height);
    ...
    //控件2
    self.textField.frame = CGRectMake(CGRectGetMaxX(self.labelLeft.frame) + kPadding, CGRectGetMinY(self.imgViewRight.frame), self.width - CGRectGetMaxX(self.labelLeft.frame) - kPadding - kX_GAP, textFieldH);
    if (!self.labelLeft.text && !self.labelLeft.attributedText) {
        self.textField.frame = CGRectMake(kX_GAP, CGRectGetMinY(self.imgViewRight.frame), self.width - kX_GAP*2, textFieldH);
